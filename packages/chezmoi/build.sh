...
PKG_FILE="$REPO_DIR/chezmoi-${LATEST_VERSION}-x86_64-1_unRAID.txz"

# Idempotent: skip rebuild if package for latest version already exists
if [ -f "$PKG_FILE" ]; then
  echo "Chezmoi $LATEST_VERSION already built at $PKG_FILE. Skipping rebuild."
  exit 0
fi
...
if compgen -G "./chezmoi-${LATEST_VERSION}-*.txz" > /dev/null; then
  # Prefer x86_64-1_unRAID naming if present
  if [ -f "./chezmoi-${LATEST_VERSION}-x86_64-1_unRAID.txz" ]; then
    mv -f "./chezmoi-${LATEST_VERSION}-x86_64-1_unRAID.txz" "$PKG_FILE"
  else
    # Fallback: move the first matching .txz into the repo, keeping its name
    FIRST_MATCH=$(ls -1 "./chezmoi-${LATEST_VERSION}-"*.txz | head -n 1)
